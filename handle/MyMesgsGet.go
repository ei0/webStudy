...
func MyMesgsGet(c *gin.Context) {
	var page = c.Query("id")
	fmt.Println("id==", page)
	var uid = c.Query("uid")
	fmt.Println("我是uid", uid)
	...
	sql.SDB.Debug().Table("messages").Where("uid=?", uid).Offset(start).Limit(LIMIT).Find(&messages).Limit(-1).Count(&count)
